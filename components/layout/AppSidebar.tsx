...
import { LayoutDashboard, Target, Award, Briefcase, Map, Sparkles, LogOut, Zap } from 'lucide-react'
...
export function AppSidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-60 h-screen sticky top-0 bg-surface-container-low border-r border-outline-variant/20 shrink-0">
      {/* Logo */}
          ...
          return (
            <Link key={href} href={href}>
              <motion.div whileHover={{ x: 2 }} whileTap={{ scale: 0.97 }}
                className={cn(
                  'flex items-center gap-sm px-sm py-xs rounded-lg text-body-md transition-all',
                  active ? 'bg-primary/10 text-primary font-semibold' : 'text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high'
                )}>
                <Icon className="w-4 h-4 shrink-0" />
                <span>{label}</span>
                {active && <motion.div layoutId="sidebar-indicator" className="ml-auto w-1 h-4 rounded-full bg-primary" />}
              </motion.div>
            </Link>
          )
        })}
      ...
      {/* User */}
      <div className="px-sm pb-sm border-t border-outline-variant/10 pt-sm">
        <div className="flex items-center gap-sm px-sm py-xs">
          ...
          <button onClick={() => { logout(); router.push('/login') }} className="text-on-surface-variant hover:text-error transition-colors p-1 rounded" title="Sign out">
