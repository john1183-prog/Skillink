...
export default function DashboardPage() {
  ...
  const readinessScore = isPythonVerified ? 72 : 64
    ...
    <div className="max-w-4xl mx-auto space-y-md">
      <FadeSlideIn>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="font-sans font-black text-headline-md text-on-surface">{greeting}, {firstName} 👋</h1>
            <p className="text-body-md text-on-surface-variant mt-xs">
              {isPythonVerified ? 'Python verified! Keep building your passport.' : 'Start verifying skills to boost your readiness.'}
            </p>
          </div>
        </div>
      </FadeSlideIn>
          ...
          <div className="p-md flex flex-col sm:flex-row items-center gap-md">
            ...
            <div className="flex-1 space-y-sm">
              <div>
                ...
                <p className="font-sans font-bold text-body-lg text-on-surface mt-0.5">{selectedPath?.title ?? 'Data Analyst'}</p>
                <div className="flex flex-wrap gap-xs mt-xs">
                  {(selectedPath?.requiredSkills ?? ['Python','SQL','Data Visualization']).slice(0,3).map(s => (
                    <Badge key={s} variant={verifiedSkills.some(v => v.skillName.toLowerCase().includes(s.toLowerCase())) ? 'verified' : 'secondary'}>{s}</Badge>
                  ))}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-sm">
                ...
                <div className="bg-surface-container rounded-lg p-sm">
                  ...
                  <p className="font-black text-2xl text-error font-mono mt-0.5">{skillGaps.filter(g => !verifiedSkills.some(v => v.skillName === g.skill)).length}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <FadeSlideIn delay={0.2}>
        <h2 className="font-mono text-label-md text-on-surface-variant uppercase tracking-wider">Quick Access</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-sm mt-sm">
          <StaggerList>
            {CARDS.map(({ title, desc, icon: Icon, href, color }) => (
              <Link key={href} href={href}>
                <motion.div whileHover={{ y: -2, boxShadow: '0 8px 32px rgba(0,56,209,0.12)' }} whileTap={{ scale: 0.97 }}
                  className="bg-surface-container-lowest border border-outline-variant/20 rounded-xl p-sm flex items-center gap-sm transition-all hover:border-primary/20">
